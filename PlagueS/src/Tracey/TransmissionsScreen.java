...
public class TransmissionsScreen extends Upgrades {
	...
	public void initItems(List<Visible> viewObjects) {
		...
		views = new ArrayList();
		double[] pts ={9.0, 12.0, 8.0, 9.0, 7.0, 10.0, 9.0};
		...
		for(int j=0; j<resources.length; j++){
			...
			TextColoredLabel dnp = new TextColoredLabel(900,390,200,40,"-"+(int)(pts[j])+" DNA points", Color.black, Color.white);
			des.setSize(20);
			det.setSize(16);
			dnp.setSize(16);

			int temp = j;
			UpgradeGraphic pic = new UpgradeGraphic(70, 180+(j*80), "Images/trans/" + resources[j].getName());
